const ProfilesEnum = {
  CLINICAL_ASSESSMENT: { id: 'clinical_assessment', title: 'Clinical Assessment', icon: '/health_report/bmi.png' },
  LIPID: { id: 'lipid_panel', title: 'Lipid Profile', icon: '/health_report/lipid.png' },
  DIABETIC: { id: 'diabetic_panel', title: 'Diabetic Mellitus Profile', icon: '/health_report/diabetic.png' },
  LIVER: { id: 'liver_panel', title: 'Liver Panel', icon: '/health_report/liver.png' },
  RENAL: { id: 'renal_profile', title: 'Kidney Profile', icon: '/health_report/kidney.png' },
  BONE_JOINT: { id: 'bone_joint_profile', title: 'Bone & Joint Profile', icon: '/health_report/bone_joint.png' },
  HAEMETOLOGY: { id: 'haematology', title: 'Haematology', icon: '/health_report/blood.png' },
  THYROID: { id: 'thyroid_function_test', title: 'Thyroid Function Test', icon: '/health_report/thyroid.png' },
  HEPATITIS: { id: 'hepatitis_profile', title: 'Hepatitis Profile', icon: '/health_report/hepatitis.png' },
  CARDIAC_RISK: { id: 'cardiac_risk_panel', title: 'Cardiac Risk Panel', icon: '/health_report/cardiac_risk.png' },
  TUMOUR_MARKERS: { id: 'tumour_markers', title: 'Tumor Marker Profile', icon: '/health_report/tumor_markers.png' },
  STD: { id: 'std_screen', title: 'STD Screen', icon: '/health_report/std.png' },
  ANAEMIA: { id: 'anaemia_profile', title: 'Anaemia Profile', icon: '/health_report/anaemia.png' },
  OTHERS: { id: 'others', title: 'Others', icon: '/health_report/others.png' },
  URINE_ANALYSIS: { id: 'urine_analysis', title: 'Urine Analysis', icon: '/health_report/urine_analysis.png' },
  STOOL_ANALYSIS: { id: 'stool_analysis', title: 'Stool Analysis', icon: '/health_report/stool.png' },
}

export const getProfileIcon = (id: string) => `https://yaadelemrtuxfyxayxpu.supabase.co/storage/v1/object/public/uploads/health_reports/icons/${id}.png`

const Profiles: Record<string, string> = Object.keys(ProfilesEnum).reduce((acc, key) => {
  acc[key] = ProfilesEnum[key].id
  return acc
}, {})

export const profiles = Object.keys(ProfilesEnum).reduce((acc, key) => {
  acc[ProfilesEnum[key].id] = ProfilesEnum[key]
  return acc
}, {})
...
const tests = healthReportMapping.map((profile) => profile.tests).flat()
export const getTest = (test_code: string) => tests.find((test) => test.test_code === test_code)
