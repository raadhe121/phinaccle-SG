...
import {
  ...
  SGiMedICType,
  SGiMedLanguage,
  updateProfileApiUserProfilePost,
    ...
    >
      ...
      >
        <TouchableHighlight
          ...
          onPress={() => Linking.openURL("tel:62351852")}
        >
          <View
            style={{
              margin: 12,
          ...
          >
            ...
            />
            <CText size={13} style={{ color: colors.warning }}>
              6235 1852
            </CText>
          </View>
        </TouchableHighlight>
...
export default function RegisterScreen() {
  ...
  const validateId = (idType: SGiMedICType, nric: string) => {
    ...
    else if (nric.length > 0 && !idValidators[idType].test(nric))
      idErrors.nric = "Please enter a valid " + (idLabel[idType] ?? "ID No.");
      ...
      <ReactQueryChild query={qry}>
        <Section title={<UpdateProfileBanner />}>
          ...
          <Label label="Nationality" wrap={false} error={errors?.nationality}>
            <FormPicker
              disabled
              data={$SGiMedNationality.enum.map((n) => ({
                label: n,
                value: n,
              }))}
              value={nationality}
