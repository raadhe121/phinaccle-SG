...
import { NavHeader3, CText, Height, Section, BoldText, TitleText, ReactQueryChild } from "@/common/components/AntdText";
import KeyboardView from "@/common/components/KeyboardView";
import { colors } from "@/common/utils/config";
import { router } from "expo-router";
import { View, TouchableHighlight, StyleSheet } from "react-native";
import { Button, Toast } from "@ant-design/react-native";
import { useAppointmentStore, MAX_MONTHS_AHEAD } from "@/hooks/useAppointment";
import dayjs from "dayjs";
import AntdMiniIcon from "@/common/components/AntdMiniIcon";
import { useQuery } from "@tanstack/react-query";
import { getAppointmentTimingsApiAppointmentV1TimingsPost } from "@/services/client";
        ...
        >
          ...
          {day.isAvailable && (
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={{ backgroundColor: availableColor, width: 6, height: 6, borderRadius: 3, marginTop: 4 }} />
              {/* <CText size={9} style={
                [
                ...
                { day.isAvailable && day.availableSlots }
              </CText> */}
            </View>
          )}
        </>
...
export default function DateTimeSelectScreen() {
  const { patients, serviceGroups, location, setStartDateTime } = useAppointmentStore();  
  ...
  const appointmentQuery = useQuery({
    queryKey: ['appointmentAvailability', selectedMonthDate.toISOString()],
    queryFn: () => getAppointmentTimingsApiAppointmentV1TimingsPost({
      requestBody: {
        service_ids: (serviceGroups?.map((group) => group.items?.map((item) => item.id) || []) || []).flat(),
        num_patients: patients?.length || 1,
  ...
  const goToPreviousMonth = () => {
    const newDate = selectedMonthDate.subtract(1, 'month').date(2);
  ...
  const goToNextMonth = () => {
    const newDate = selectedMonthDate.add(1, 'month').date(2);
