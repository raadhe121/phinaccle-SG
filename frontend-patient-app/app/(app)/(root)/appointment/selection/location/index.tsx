...
import { BoldText, NavHeader3, CMarkdown, Row } from "@/common/components/AntdText"
import KeyboardView from "@/common/components/KeyboardView"
import { router } from "expo-router"
import { useQuery } from "@tanstack/react-query"
import { View, ActivityIndicator, ScrollView, Alert, TouchableHighlight, ViewStyle } from "react-native";
import { colors } from "@/common/utils/config";
import { useAppointmentStore } from '@/hooks/useAppointment'
import { BranchList } from "./branch";
import { Route, TabBar, TabView } from "react-native-tab-view";
import { ApptBranch, getLocationsApiAppointmentV1LocationsPost, GetLocationsResp } from "@/services/client";

// Location selection screen for appointments
export default function LocationSelectScreen() {
  const { serviceGroups, corporateCode, branchIds } = useAppointmentStore();

  const services = serviceGroups?.map((sg) => sg.items?.map((i) => i.id).join(',')).join(',');
  const qry = useQuery({
    queryKey: ['appointment', 'branches', services, corporateCode?.code, branchIds],
    queryFn: ({ queryKey }) => getLocationsApiAppointmentV1LocationsPost({
      requestBody: {
...
export function ClinicLocations({ data }: { data: GetLocationsResp }) {
  ...
  const branchesByCategory: { [key: string]: ApptBranch[] } = data?.branches.reduce((acc, b) => {
    if (!acc[b.category]) {
      acc[b.category] = []
    }
  ...
  return (
    <TabView
      ...
      renderScene={({ route }: { route: Route }) => {
        ...
        return <BranchList type={data.type} branches={data.branches.filter((b) => b.category === route.key)} />
      }}
    />
  )
}
...
export function OnsiteLocations({ data }: { data: GetLocationsResp }) {
  ...
  const BranchButton = ({ onPress, children, disabled = false, style }: { onPress: () => void, children: React.ReactNode, disabled?: boolean, style?: ViewStyle }) => {
    return <TouchableHighlight onPress={onPress} underlayColor={colors.underlay} style={{ borderWidth: disabled ? 0 : 1, borderColor: colors.brands2, borderRadius: 8, ...style }}>
        <View style={{ marginHorizontal: 20, marginVertical: 8}}>
          <BoldText style={{ color: disabled ? colors.weak : colors.brands2 }}>{children}</BoldText>
        </View>
      </TouchableHighlight>
  }
  ...
  return (
    <>
      <Row style={{ margin: 12 }}>
        <BranchButton onPress={() => {}}>
          On-site Services
        </BranchButton>
        <BranchButton onPress={handleAllClinicsPress} disabled={true} style={{ marginLeft: 12 }}>
          All Clinics
        </BranchButton>
      </Row>
      <BranchList
