...
export const BranchList = ({
  ...
  const branchesByCategory: { [key: string]: ApptBranch[] } = branches.reduce(
    (acc, b) => {
      if (!acc[b.category]) {
        acc[b.category] = [];
      }
  ...
  return (
    <ScrollView>
      {header}
      {Object.keys(branchesByCategory)
        .sort()
        .map((category) => (
          <Section
            key={category}
          ...
          >
            {branchesByCategory[category].map((b) => {
              if (branchId === b.id) {
                return (
                  <View key={b.id} style={{ backgroundColor: colors.brands4 }}>
                    <ListItemView onPress={() => setBranchId(undefined)}>
                    ...
                    <BranchDetails type={type} branch={b} />
                  </View>
                );
              }

              return (
                <ListItemView key={b.id} onPress={() => setBranchId(b.id)}>
                  <CText size={16} style={{ marginTop: 14, marginBottom: 14 }}>
                    {b.name}
                  </CText>
                </ListItemView>
              );
